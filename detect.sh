...
echo $(date "+%Y%m%d %H:%M:%S.%N")" $SCRIPT:GetDateTime loop start, car_id=$1"
while [ 1 -eq 1 ];do
...
echo $(date "+%Y%m%d %H:%M:%S.%N")" $SCRIPT:GetDateTime tigger create, tigger_time=$TIGGER"
break
else
...
echo $(date "+%Y%m%d %H:%M:%S.%N")" $SCRIPT:GetDateTime loop end"
